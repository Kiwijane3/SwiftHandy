...
///
/// The `SwipeableProtocol` protocol exposes the methods and properties of an underlying `HdySwipeable` instance.
...
public protocol SwipeableProtocol: Gtk.WidgetProtocol {
        /// Untyped pointer to the underlying `HdySwipeable` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdySwipeable` instance.
    var swipeable_ptr: UnsafeMutablePointer<HdySwipeable>! { get }
...
///
/// The `SwipeableRef` type acts as a lightweight Swift reference to an underlying `HdySwipeable` instance.
/// It exposes methods that can operate on this data type through `SwipeableProtocol` conformance.
/// Use `SwipeableRef` only as an `unowned` reference to an existing `HdySwipeable` instance.
///
...
public extension SwipeableRef {
    ...
    @inlinable init(_ p: UnsafeMutablePointer<HdySwipeable>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdySwipeable>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdySwipeable>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdySwipeable>?) {
...
///
/// The `Swipeable` type acts as a reference-counted owner of an underlying `HdySwipeable` instance.
/// It provides the methods that can operate on this data type through `SwipeableProtocol` conformance.
/// Use `Swipeable` as a strong reference or owner of a `HdySwipeable` instance.
///
open class Swipeable: Gtk.Widget, SwipeableProtocol {
    ...
    @inlinable public init(_ op: UnsafeMutablePointer<HdySwipeable>) {
    ...
    /// i.e., ownership is transferred to the `Swipeable` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdySwipeable>) {
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdySwipeable`.
    /// i.e., ownership is transferred to the `Swipeable` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdySwipeable>) {
...
public enum SwipeableSignalName: String, SignalNameProtocol {
    ...
    case childSwitched = "child-switched"
...
// MARK: Swipeable signals
public extension SwipeableProtocol {
    /// Connect a Swift signal handler to the given, typed `SwipeableSignalName` signal
    /// - Parameters:
    ...
    @inlinable @discardableResult func connect(signal s: SwipeableSignalName, flags f: ConnectFlags = ConnectFlags(0), handler h: @escaping SignalHandler) -> Int {
        GLibObject.ObjectRef(raw: ptr).connect(s, flags: f, handler: h)
    }
    
    
    /// Connect a C signal handler to the given, typed `SwipeableSignalName` signal
    /// - Parameters:
    ///   - signal: The signal to connect
    ///   - flags: The connection flags to use
    ///   - data: A pointer to user data to provide to the callback
    ...
    /// - Returns: The signal handler ID (always greater than 0 for successful connections)
    @inlinable @discardableResult func connect(signal s: SwipeableSignalName, flags f: ConnectFlags = ConnectFlags(0), data userData: gpointer!, destroyData destructor: GClosureNotify? = nil, signalHandler h: @escaping GCallback) -> Int {
    ...
    /// 
    /// This is used by `HdySwipeGroup`, applications should not connect to it.
    /// - Note: This represents the underlying `child-switched` signal
    /// - Parameter flags: Flags
    /// - Parameter unownedSelf: Reference to instance of self
    /// - Parameter index: the index of the child to switch to
    /// - Parameter duration: Animation duration in milliseconds
    /// - Parameter handler: The signal handler to call
    /// Run the given callback whenever the `childSwitched` signal is emitted
    @discardableResult @inlinable func onChildSwitched(flags: ConnectFlags = ConnectFlags(0), handler: @escaping ( _ unownedSelf: SwipeableRef, _ index: UInt, _ duration: gint64) -> Void ) -> Int {
        typealias SwiftHandler = GLib.ClosureHolder3<SwipeableRef, UInt, gint64, Void>
        let cCallback: @convention(c) (gpointer, guint, gint64, gpointer) -> Void = { unownedSelf, arg1, arg2, userData in
            let holder = Unmanaged<SwiftHandler>.fromOpaque(userData).takeUnretainedValue()
            let output: Void = holder.call(SwipeableRef(raw: unownedSelf), UInt(arg1), arg2)
            return output
        }
        return connect(
            signal: .childSwitched,
            flags: flags,
    ...
    /// Typed `child-switched` signal for using the `connect(signal:)` methods
    static var childSwitchedSignal: SwipeableSignalName { .childSwitched }
    
    // Swipeable property signals were not generated due to unavailability of GObject during generation time
}

// MARK: Swipeable Interface: SwipeableProtocol extension (methods and fields)
public extension SwipeableProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdySwipeable` instance.
    @inlinable var swipeable_ptr: UnsafeMutablePointer<HdySwipeable>! { return ptr?.assumingMemoryBound(to: HdySwipeable.self) }

    /// Emits HdySwipeable`child-switched` signal. This should be called when the
    /// widget switches visible child widget.
    /// 
    /// `duration` can be 0 if the child is switched without animation.
    @inlinable func emitChildSwitched(index: Int, duration: gint64) {
        hdy_swipeable_emit_child_switched(swipeable_ptr, guint(index), duration)
    
    }

    /// Gets the progress `self` will snap back to after the gesture is canceled.
    @inlinable func getCancelProgress() -> Double {
        let rv = Double(hdy_swipeable_get_cancel_progress(swipeable_ptr))
        return rv
    }

    /// Gets the swipe distance of `self`. This corresponds to how many pixels
    /// 1 unit represents.
    @inlinable func getDistance() -> Double {
        let rv = Double(hdy_swipeable_get_distance(swipeable_ptr))
        return rv
    }

    /// Gets the current progress of `self`
    @inlinable func getProgress() -> Double {
        let rv = Double(hdy_swipeable_get_progress(swipeable_ptr))
        return rv
    }
    ...
    @inlinable func getSnapPoints(nSnapPoints: UnsafeMutablePointer<gint>!) -> UnsafeMutablePointer<gdouble>! {
        let rv = hdy_swipeable_get_snap_points(swipeable_ptr, nSnapPoints)
        return rv
    }
    ...
    @inlinable func getSwipeArea<RectangleT: Gdk.RectangleProtocol>(navigationDirection: HdyNavigationDirection, isDrag: Bool, rect: RectangleT) {
        hdy_swipeable_get_swipe_area(swipeable_ptr, navigationDirection, gboolean((isDrag) ? 1 : 0), rect.rectangle_ptr)
    
    }

    /// Gets the `HdySwipeTracker` used by this swipeable widget.
    @inlinable func getSwipeTracker() -> SwipeTrackerRef! {
        let rv = SwipeTrackerRef(gconstpointer: gconstpointer(hdy_swipeable_get_swipe_tracker(swipeable_ptr)))
        return rv
    }

    /// See HdySwipeable`child-switched`.
    @inlinable func switchChild(index: Int, duration: gint64) {
        hdy_swipeable_switch_child(swipeable_ptr, guint(index), duration)
    
    }
    /// Gets the progress `self` will snap back to after the gesture is canceled.
    @inlinable var cancelProgress: Double {
        /// Gets the progress `self` will snap back to after the gesture is canceled.
        get {
            let rv = Double(hdy_swipeable_get_cancel_progress(swipeable_ptr))
            return rv
    ...
    /// Gets the `HdySwipeTracker` used by this swipeable widget.
    @inlinable var swipeTracker: SwipeTrackerRef! {
        /// Gets the `HdySwipeTracker` used by this swipeable widget.
        get {
            let rv = SwipeTrackerRef(gconstpointer: gconstpointer(hdy_swipeable_get_swipe_tracker(swipeable_ptr)))
            return rv
