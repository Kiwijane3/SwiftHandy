...
///
/// The `ActionRowProtocol` protocol exposes the methods and properties of an underlying `HdyActionRow` instance.
...
public protocol ActionRowProtocol: PreferencesRowProtocol {
        /// Untyped pointer to the underlying `HdyActionRow` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyActionRow` instance.
    var action_row_ptr: UnsafeMutablePointer<HdyActionRow>! { get }
...
///
/// The `ActionRowRef` type acts as a lightweight Swift reference to an underlying `HdyActionRow` instance.
/// It exposes methods that can operate on this data type through `ActionRowProtocol` conformance.
/// Use `ActionRowRef` only as an `unowned` reference to an existing `HdyActionRow` instance.
///
...
public extension ActionRowRef {
    ...
    @inlinable init(_ p: UnsafeMutablePointer<HdyActionRow>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyActionRow>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyActionRow>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyActionRow>?) {
...
///
/// The `ActionRow` type acts as a reference-counted owner of an underlying `HdyActionRow` instance.
...
open class ActionRow: PreferencesRow, ActionRowProtocol {
    ...
    @inlinable public init(_ op: UnsafeMutablePointer<HdyActionRow>) {
    ...
    /// i.e., ownership is transferred to the `ActionRow` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyActionRow>) {
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyActionRow`.
    /// i.e., ownership is transferred to the `ActionRow` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyActionRow>) {
...
public extension ActionRowProtocol {
    ...
    // ActionRow property signals were not generated due to unavailability of GObject during generation time
}

// MARK: ActionRow Class: ActionRowProtocol extension (methods and fields)
public extension ActionRowProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyActionRow` instance.
    @inlinable var action_row_ptr: UnsafeMutablePointer<HdyActionRow>! { return ptr?.assumingMemoryBound(to: HdyActionRow.self) }

    @inlinable func activate() {
        hdy_action_row_activate(action_row_ptr)
    
    }

    /// Adds a prefix widget to `self`.
    @inlinable func addPrefix<WidgetT: Gtk.WidgetProtocol>(widget: WidgetT) {
        hdy_action_row_add_prefix(action_row_ptr, widget.widget_ptr)
    
    }

    /// Gets the widget activated when `self` is activated.
    @inlinable func getActivatableWidget() -> Gtk.WidgetRef! {
        let rv = Gtk.WidgetRef(hdy_action_row_get_activatable_widget(action_row_ptr))
        return rv
    }

    /// Gets the icon name for `self`.
    @inlinable func getIconName() -> String! {
        let rv = hdy_action_row_get_icon_name(action_row_ptr).map({ String(cString: $0) })
        return rv
    }

    /// Gets the subtitle for `self`.
    @inlinable func getSubtitle() -> String! {
        let rv = hdy_action_row_get_subtitle(action_row_ptr).map({ String(cString: $0) })
        return rv
    }
    ...
    @inlinable func getSubtitleLines() -> Int {
        let rv = Int(hdy_action_row_get_subtitle_lines(action_row_ptr))
        return rv
    }
    ...
    /// ellipsized.
    /// If the value is 0, the number of lines won't be limited.
    @inlinable func getTitleLines() -> Int {
        let rv = Int(hdy_action_row_get_title_lines(action_row_ptr))
        return rv
    }
    ...
    @inlinable func getUseUnderline() -> Bool {
        let rv = ((hdy_action_row_get_use_underline(action_row_ptr)) != 0)
        return rv
    }
    ...
    @inlinable func setActivatable(widget: Gtk.WidgetRef? = nil) {
        hdy_action_row_set_activatable_widget(action_row_ptr, widget?.widget_ptr)
    ...
    /// The target widget will be activated by emitting the
    /// GtkWidget`mnemonic-activate` signal on it.
    @inlinable func setActivatable<WidgetT: Gtk.WidgetProtocol>(widget: WidgetT?) {
        hdy_action_row_set_activatable_widget(action_row_ptr, widget?.widget_ptr)
    ...
    @inlinable func set(iconName: UnsafePointer<gchar>!) {
        hdy_action_row_set_icon_name(action_row_ptr, iconName)
    
    }

    /// Sets the subtitle for `self`.
    @inlinable func set(subtitle: UnsafePointer<gchar>? = nil) {
        hdy_action_row_set_subtitle(action_row_ptr, subtitle)
    ...
    /// ellipsized.
    /// If the value is 0, the number of lines won't be limited.
    @inlinable func set(subtitleLines: Int) {
        hdy_action_row_set_subtitle_lines(action_row_ptr, gint(subtitleLines))
    ...
    /// ellipsized.
    /// If the value is 0, the number of lines won't be limited.
    @inlinable func set(titleLines: Int) {
        hdy_action_row_set_title_lines(action_row_ptr, gint(titleLines))
    ...
    @inlinable func set(useUnderline: Bool) {
        hdy_action_row_set_use_underline(action_row_ptr, gboolean((useUnderline) ? 1 : 0))
    ...
    @inlinable var activatableWidget: Gtk.WidgetRef! {
        ...
        /// GtkWidget`mnemonic-activate` signal on it.
        nonmutating set {
            hdy_action_row_set_activatable_widget(action_row_ptr, UnsafeMutablePointer<GtkWidget>(newValue?.widget_ptr))
        }
    }
    ...
    @inlinable var iconName: String! {
        ...
        /// Sets the icon name for `self`.
        nonmutating set {
            hdy_action_row_set_icon_name(action_row_ptr, newValue)
        }
    }
    ...
    @inlinable var subtitle: String! {
        ...
        /// Sets the subtitle for `self`.
        nonmutating set {
            hdy_action_row_set_subtitle(action_row_ptr, newValue)
        }
    }
    ...
    /// ellipsized.
    /// If the value is 0, the number of lines won't be limited.
    @inlinable var subtitleLines: Int {
        /// Gets the number of lines at the end of which the subtitle label will be
        /// ellipsized.
        /// If the value is 0, the number of lines won't be limited.
        get {
        ...
        /// If the value is 0, the number of lines won't be limited.
        nonmutating set {
            hdy_action_row_set_subtitle_lines(action_row_ptr, gint(newValue))
        }
    }
    ...
    @inlinable var titleLines: Int {
        ...
        /// If the value is 0, the number of lines won't be limited.
        nonmutating set {
            hdy_action_row_set_title_lines(action_row_ptr, gint(newValue))
        }
    }
    ...
    @inlinable var useUnderline: Bool {
        ...
        /// the next character should be used for the mnemonic accelerator key.
        nonmutating set {
            hdy_action_row_set_use_underline(action_row_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
...
///
/// The `ApplicationWindowProtocol` protocol exposes the methods and properties of an underlying `HdyApplicationWindow` instance.
/// The default implementation of these can be found in the protocol extension below.
...
///
public protocol ApplicationWindowProtocol: Gtk.ApplicationWindowProtocol, Gtk.BuildableProtocol {
        /// Untyped pointer to the underlying `HdyApplicationWindow` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyApplicationWindow` instance.
    var application_window_ptr: UnsafeMutablePointer<HdyApplicationWindow>! { get }
...
///
/// The `ApplicationWindowRef` type acts as a lightweight Swift reference to an underlying `HdyApplicationWindow` instance.
/// It exposes methods that can operate on this data type through `ApplicationWindowProtocol` conformance.
/// Use `ApplicationWindowRef` only as an `unowned` reference to an existing `HdyApplicationWindow` instance.
///
...
public extension ApplicationWindowRef {
    /// Designated initialiser from the underlying `C` data type
    @inlinable init(_ p: UnsafeMutablePointer<HdyApplicationWindow>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyApplicationWindow>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyApplicationWindow>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyApplicationWindow>?) {
        guard let p = UnsafeMutablePointer(mutating: maybePointer) else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }
...
///
/// The `ApplicationWindow` type acts as a reference-counted owner of an underlying `HdyApplicationWindow` instance.
...
open class ApplicationWindow: Gtk.ApplicationWindow, ApplicationWindowProtocol {
    ...
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<HdyApplicationWindow>) {
        super.init(cPointer: op)
    }
    ...
    /// i.e., ownership is transferred to the `ApplicationWindow` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyApplicationWindow>) {
        super.init(raw: UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op)))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyApplicationWindow`.
    /// i.e., ownership is transferred to the `ApplicationWindow` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyApplicationWindow>) {
        super.init(retainingCPointer: op)
    }
    ...
    @inlinable public init<T: ApplicationWindowProtocol>(handyApplicationWindow other: T) {
        super.init(retainingRaw: other.ptr)
    }
...
public extension ApplicationWindowProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyApplicationWindow` instance.
    @inlinable var application_window_ptr: UnsafeMutablePointer<HdyApplicationWindow>! { return ptr?.assumingMemoryBound(to: HdyApplicationWindow.self) }
...
///
/// The `AvatarProtocol` protocol exposes the methods and properties of an underlying `HdyAvatar` instance.
/// The default implementation of these can be found in the protocol extension below.
...
///
public protocol AvatarProtocol: Gtk.DrawingAreaProtocol, Gtk.BuildableProtocol {
        /// Untyped pointer to the underlying `HdyAvatar` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyAvatar` instance.
    var avatar_ptr: UnsafeMutablePointer<HdyAvatar>! { get }
...
///
/// The `AvatarRef` type acts as a lightweight Swift reference to an underlying `HdyAvatar` instance.
/// It exposes methods that can operate on this data type through `AvatarProtocol` conformance.
/// Use `AvatarRef` only as an `unowned` reference to an existing `HdyAvatar` instance.
///
...
public extension AvatarRef {
    /// Designated initialiser from the underlying `C` data type
    @inlinable init(_ p: UnsafeMutablePointer<HdyAvatar>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyAvatar>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyAvatar>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyAvatar>?) {
        guard let p = UnsafeMutablePointer(mutating: maybePointer) else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }
...
///
/// The `Avatar` type acts as a reference-counted owner of an underlying `HdyAvatar` instance.
...
open class Avatar: Gtk.DrawingArea, AvatarProtocol {
    ...
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<HdyAvatar>) {
        super.init(cPointer: op)
    }
    ...
    /// i.e., ownership is transferred to the `Avatar` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyAvatar>) {
        super.init(raw: UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op)))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyAvatar`.
    /// i.e., ownership is transferred to the `Avatar` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyAvatar>) {
        super.init(retainingCPointer: op)
    }
...
public enum AvatarPropertyName: String, PropertyNameProtocol {
    ...
    case loadableIcon = "loadable-icon"
    /// Sets all four sides' margin at once. If read, returns max
    /// margin on any side.
    case margin = "margin"
    /// Margin on bottom side of widget.
...
public enum AvatarSignalName: String, SignalNameProtocol {
    ...
    case notifyIsFocus = "notify::is-focus"
    /// A `GLoadableIcon` used to load the avatar.
    case notifyLoadableIcon = "notify::loadable-icon"
    /// Sets all four sides' margin at once. If read, returns max
    /// margin on any side.
    case notifyMargin = "notify::margin"
    /// Margin on bottom side of widget.
...
public extension AvatarProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyAvatar` instance.
    @inlinable var avatar_ptr: UnsafeMutablePointer<HdyAvatar>! { return ptr?.assumingMemoryBound(to: HdyAvatar.self) }

    /// Renders `self` into a pixbuf at `size` and `scale_factor`. This can be used to export the fallback avatar.
    @inlinable func drawToPixbuf(size: Int, scaleFactor: Int) -> PixbufRef! {
        let rv = PixbufRef(gconstpointer: gconstpointer(hdy_avatar_draw_to_pixbuf(avatar_ptr, gint(size), gint(scaleFactor))))
        return rv
    }

    /// Renders asynchronously `self` into a pixbuf at `size` and `scale_factor`.
    /// This can be used to export the fallback avatar.
    @inlinable func drawToPixbufAsync(size: Int, scaleFactor: Int, cancellable: GIO.CancellableRef? = nil, callback: GAsyncReadyCallback? = nil, userData: gpointer! = nil) {
        hdy_avatar_draw_to_pixbuf_async(avatar_ptr, gint(size), gint(scaleFactor), cancellable?.cancellable_ptr, callback, userData)
    
    }
    /// Renders asynchronously `self` into a pixbuf at `size` and `scale_factor`.
    /// This can be used to export the fallback avatar.
    @inlinable func drawToPixbufAsync<CancellableT: GIO.CancellableProtocol>(size: Int, scaleFactor: Int, cancellable: CancellableT?, callback: GAsyncReadyCallback? = nil, userData: gpointer! = nil) {
        hdy_avatar_draw_to_pixbuf_async(avatar_ptr, gint(size), gint(scaleFactor), cancellable?.cancellable_ptr, callback, userData)
    
    }

    /// Finishes an asynchronous draw of an avatar to a pixbuf.
    @inlinable func drawToPixbufFinish<AsyncResultT: GIO.AsyncResultProtocol>(asyncResult: AsyncResultT) -> PixbufRef! {
        let rv = PixbufRef(gconstpointer: gconstpointer(hdy_avatar_draw_to_pixbuf_finish(avatar_ptr, asyncResult.async_result_ptr)))
        return rv
    }
    ...
    /// displayed.
    @inlinable func getIconName() -> String! {
        let rv = hdy_avatar_get_icon_name(avatar_ptr).map({ String(cString: $0) })
        return rv
    }

    /// Gets the `GLoadableIcon` set via `hdy_avatar_set_loadable_icon()`.
    @inlinable func getLoadableIcon() -> GIO.LoadableIconRef! {
        let rv = GIO.LoadableIconRef(hdy_avatar_get_loadable_icon(avatar_ptr))
        return rv
    }

    /// Returns whether initials are used for the fallback or the icon.
    @inlinable func getShowInitials() -> Bool {
        let rv = ((hdy_avatar_get_show_initials(avatar_ptr)) != 0)
        return rv
    }

    /// Returns the size of the avatar.
    @inlinable func getSize() -> Int {
        let rv = Int(hdy_avatar_get_size(avatar_ptr))
        return rv
    }

    /// Get the text used to generate the fallback initials and color
    @inlinable func getText() -> String! {
        let rv = hdy_avatar_get_text(avatar_ptr).map({ String(cString: $0) })
        return rv
    }
    ...
    @inlinable func set(iconName: UnsafePointer<gchar>? = nil) {
        hdy_avatar_set_icon_name(avatar_ptr, iconName)
    ...
    ///
    /// **set_image_load_func is deprecated:**
    /// use hdy_avatar_set_loadable_icon() instead.
    @available(*, deprecated)
    @inlinable func setImageLoadFunc(loadImage: HdyAvatarImageLoadFunc? = nil, userData: gpointer! = nil, destroy: GDestroyNotify? = nil) {
        hdy_avatar_set_image_load_func(avatar_ptr, loadImage, userData, destroy)
    
    }

    /// Sets the `GLoadableIcon` to use as an avatar.
    /// The previous avatar is displayed till the new avatar is loaded,
    /// to immediately remove the custom avatar set the loadable-icon to `nil`.
    /// 
    /// The `GLoadableIcon` set via this function is prefered over a set `HdyAvatarImageLoadFunc`.
    @inlinable func setLoadable(icon: GIO.LoadableIconRef? = nil) {
        hdy_avatar_set_loadable_icon(avatar_ptr, icon?.loadable_icon_ptr)
    ...
    /// 
    /// The `GLoadableIcon` set via this function is prefered over a set `HdyAvatarImageLoadFunc`.
    @inlinable func setLoadable<LoadableIconT: GIO.LoadableIconProtocol>(icon: LoadableIconT?) {
        hdy_avatar_set_loadable_icon(avatar_ptr, icon?.loadable_icon_ptr)
    ...
    @inlinable func set(showInitials: Bool) {
        hdy_avatar_set_show_initials(avatar_ptr, gboolean((showInitials) ? 1 : 0))
    
    }

    /// Sets the size of the avatar.
    @inlinable func set(size: Int) {
        hdy_avatar_set_size(avatar_ptr, gint(size))
    
    }

    /// Set the text used to generate the fallback initials color
    @inlinable func set(text: UnsafePointer<gchar>? = nil) {
        hdy_avatar_set_text(avatar_ptr, text)
    ...
    @inlinable var iconName: String! {
        ...
        /// If the icon theme is changed, the image will be updated automatically.
        nonmutating set {
            hdy_avatar_set_icon_name(avatar_ptr, newValue)
        }
    }

    /// Gets the `GLoadableIcon` set via `hdy_avatar_set_loadable_icon()`.
    @inlinable var loadableIcon: GIO.LoadableIconRef! {
        /// Gets the `GLoadableIcon` set via `hdy_avatar_set_loadable_icon()`.
        get {
            let rv = GIO.LoadableIconRef(hdy_avatar_get_loadable_icon(avatar_ptr))
            return rv
        ...
        /// The `GLoadableIcon` set via this function is prefered over a set `HdyAvatarImageLoadFunc`.
        nonmutating set {
            hdy_avatar_set_loadable_icon(avatar_ptr, UnsafeMutablePointer<GLoadableIcon>(newValue?.loadable_icon_ptr))
        }
    }
    ...
    @inlinable var showInitials: Bool {
        ...
        /// Sets whether the initials should be shown on the fallback avatar or the icon.
        nonmutating set {
            hdy_avatar_set_show_initials(avatar_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
    ...
    @inlinable var size: Int {
        ...
        /// Sets the size of the avatar.
        nonmutating set {
            hdy_avatar_set_size(avatar_ptr, gint(newValue))
        }
    }
    ...
    @inlinable var text: String! {
        ...
        /// Set the text used to generate the fallback initials color
        nonmutating set {
            hdy_avatar_set_text(avatar_ptr, newValue)
        }
    }
...
///
/// The `CarouselProtocol` protocol exposes the methods and properties of an underlying `HdyCarousel` instance.
/// The default implementation of these can be found in the protocol extension below.
...
///
public protocol CarouselProtocol: Gtk.EventBoxProtocol, Gtk.BuildableProtocol, Gtk.OrientableProtocol, SwipeableProtocol {
        /// Untyped pointer to the underlying `HdyCarousel` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyCarousel` instance.
    var carousel_ptr: UnsafeMutablePointer<HdyCarousel>! { get }
...
///
/// The `CarouselRef` type acts as a lightweight Swift reference to an underlying `HdyCarousel` instance.
/// It exposes methods that can operate on this data type through `CarouselProtocol` conformance.
/// Use `CarouselRef` only as an `unowned` reference to an existing `HdyCarousel` instance.
///
...
public extension CarouselRef {
    /// Designated initialiser from the underlying `C` data type
    @inlinable init(_ p: UnsafeMutablePointer<HdyCarousel>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyCarousel>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyCarousel>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyCarousel>?) {
        guard let p = UnsafeMutablePointer(mutating: maybePointer) else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }
...
///
/// The `Carousel` type acts as a reference-counted owner of an underlying `HdyCarousel` instance.
...
open class Carousel: Gtk.EventBox, CarouselProtocol {
    ...
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<HdyCarousel>) {
        super.init(cPointer: op)
    }
    ...
    /// i.e., ownership is transferred to the `Carousel` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyCarousel>) {
        super.init(raw: UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op)))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyCarousel`.
    /// i.e., ownership is transferred to the `Carousel` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyCarousel>) {
        super.init(retainingCPointer: op)
    }
...
public enum CarouselSignalName: String, SignalNameProtocol {
    ...
    /// Animation duration in milliseconds, used by `hdy_carousel_scroll_to()`.
    case notifyAnimationDuration = "notify::animation-duration"
    case notifyAppPaintable = "notify::app-paintable"
    case notifyBorderWidth = "notify::border-width"
    case notifyCanDefault = "notify::can-default"
    case notifyCanFocus = "notify::can-focus"
...
public extension CarouselProtocol {
    ...
    @discardableResult @inlinable func onPageChanged(flags: ConnectFlags = ConnectFlags(0), handler: @escaping ( _ unownedSelf: CarouselRef, _ index: UInt) -> Void ) -> Int {
        typealias SwiftHandler = GLib.ClosureHolder2<CarouselRef, UInt, Void>
    ...
    // Carousel property signals were not generated due to unavailability of GObject during generation time
}

// MARK: Carousel Class: CarouselProtocol extension (methods and fields)
public extension CarouselProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyCarousel` instance.
    @inlinable var carousel_ptr: UnsafeMutablePointer<HdyCarousel>! { return ptr?.assumingMemoryBound(to: HdyCarousel.self) }

    /// Whether to allow swiping for more than one page at a time. If the value is
    /// `false`, each swipe can only move to the adjacent pages.
    @inlinable func getAllowLongSwipes() -> Bool {
        let rv = ((hdy_carousel_get_allow_long_swipes(carousel_ptr)) != 0)
        return rv
    }

    /// Sets whether `self` can be dragged with mouse pointer
    @inlinable func getAllowMouseDrag() -> Bool {
        let rv = ((hdy_carousel_get_allow_mouse_drag(carousel_ptr)) != 0)
        return rv
    }

    /// Gets animation duration used by `hdy_carousel_scroll_to()`.
    @inlinable func getAnimationDuration() -> Int {
        let rv = Int(hdy_carousel_get_animation_duration(carousel_ptr))
        return rv
    }

    /// Gets whether `self` can be navigated.
    @inlinable func getInteractive() -> Bool {
        let rv = ((hdy_carousel_get_interactive(carousel_ptr)) != 0)
        return rv
    }

    /// Gets the number of pages in `self`.
    @inlinable func getNPages() -> Int {
        let rv = Int(hdy_carousel_get_n_pages(carousel_ptr))
        return rv
    }

    /// Gets current scroll position in `self`. It's unitless, 1 matches 1 page.
    @inlinable func getPosition() -> Double {
        let rv = Double(hdy_carousel_get_position(carousel_ptr))
        return rv
    }
    ...
    @inlinable func getRevealDuration() -> Int {
        let rv = Int(hdy_carousel_get_reveal_duration(carousel_ptr))
        return rv
    }

    /// Gets spacing between pages in pixels.
    @inlinable func getSpacing() -> Int {
        let rv = Int(hdy_carousel_get_spacing(carousel_ptr))
        return rv
    }
    ...
    @inlinable func insert<WidgetT: Gtk.WidgetProtocol>(child: WidgetT, position: Int) {
        hdy_carousel_insert(carousel_ptr, child.widget_ptr, gint(position))
    
    }

    /// Prepends `child` to `self`
    @inlinable func prepend<WidgetT: Gtk.WidgetProtocol>(child: WidgetT) {
        hdy_carousel_prepend(carousel_ptr, child.widget_ptr)
    ...
    /// to the end.
    @inlinable func reorder<WidgetT: Gtk.WidgetProtocol>(child: WidgetT, position: Int) {
        hdy_carousel_reorder(carousel_ptr, child.widget_ptr, gint(position))
    ...
    /// duration.
    @inlinable func scrollTo<WidgetT: Gtk.WidgetProtocol>(widget: WidgetT) {
        hdy_carousel_scroll_to(carousel_ptr, widget.widget_ptr)
    
    }

    /// Scrolls to `widget` position with an animation.
    @inlinable func scrollToFull<WidgetT: Gtk.WidgetProtocol>(widget: WidgetT, duration: gint64) {
        hdy_carousel_scroll_to_full(carousel_ptr, widget.widget_ptr, duration)
    ...
    @inlinable func set(allowLongSwipes: Bool) {
        hdy_carousel_set_allow_long_swipes(carousel_ptr, gboolean((allowLongSwipes) ? 1 : 0))
    ...
    @inlinable func set(allowMouseDrag: Bool) {
        hdy_carousel_set_allow_mouse_drag(carousel_ptr, gboolean((allowMouseDrag) ? 1 : 0))
    
    }

    /// Sets animation duration used by `hdy_carousel_scroll_to()`.
    @inlinable func setAnimation(duration: Int) {
        hdy_carousel_set_animation_duration(carousel_ptr, guint(duration))
    ...
    @inlinable func set(interactive: Bool) {
        hdy_carousel_set_interactive(carousel_ptr, gboolean((interactive) ? 1 : 0))
    ...
    @inlinable func set(revealDuration: Int) {
        hdy_carousel_set_reveal_duration(carousel_ptr, guint(revealDuration))
    
    }

    /// Sets spacing between pages in pixels.
    @inlinable func set(spacing: Int) {
        hdy_carousel_set_spacing(carousel_ptr, guint(spacing))
    ...
    @inlinable var allowLongSwipes: Bool {
        ...
        /// is `false`, each swipe can only move to the adjacent pages.
        nonmutating set {
            hdy_carousel_set_allow_long_swipes(carousel_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
    ...
    @inlinable var allowMouseDrag: Bool {
        ...
        /// is `false`, dragging is only available on touch.
        nonmutating set {
            hdy_carousel_set_allow_mouse_drag(carousel_ptr, gboolean((newValue) ? 1 : 0))
        }
    }

    /// Gets animation duration used by `hdy_carousel_scroll_to()`.
    @inlinable var animationDuration: Int {
        /// Gets animation duration used by `hdy_carousel_scroll_to()`.
        get {
            let rv = Int(hdy_carousel_get_animation_duration(carousel_ptr))
            return rv
        ...
        /// Sets animation duration used by `hdy_carousel_scroll_to()`.
        nonmutating set {
            hdy_carousel_set_animation_duration(carousel_ptr, guint(newValue))
        }
    }
    ...
    @inlinable var interactive: Bool {
        ...
        /// a `HdyCarousel` to only allow swiping in a certain state.
        nonmutating set {
            hdy_carousel_set_interactive(carousel_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
    ...
    @inlinable var revealDuration: Int {
        ...
        /// milliseconds.
        nonmutating set {
            hdy_carousel_set_reveal_duration(carousel_ptr, guint(newValue))
        }
    }
    ...
    @inlinable var spacing: Int {
        ...
        /// Sets spacing between pages in pixels.
        nonmutating set {
            hdy_carousel_set_spacing(carousel_ptr, guint(newValue))
        }
    }
