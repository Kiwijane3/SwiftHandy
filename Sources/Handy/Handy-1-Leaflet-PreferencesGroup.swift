...
///
/// The `LeafletProtocol` protocol exposes the methods and properties of an underlying `HdyLeaflet` instance.
...
///
public protocol LeafletProtocol: Gtk.ContainerProtocol, Gtk.BuildableProtocol, Gtk.OrientableProtocol, SwipeableProtocol {
        /// Untyped pointer to the underlying `HdyLeaflet` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyLeaflet` instance.
    var leaflet_ptr: UnsafeMutablePointer<HdyLeaflet>! { get }
...
///
/// The `LeafletRef` type acts as a lightweight Swift reference to an underlying `HdyLeaflet` instance.
/// It exposes methods that can operate on this data type through `LeafletProtocol` conformance.
/// Use `LeafletRef` only as an `unowned` reference to an existing `HdyLeaflet` instance.
///
...
public extension LeafletRef {
    ...
    @inlinable init(_ p: UnsafeMutablePointer<HdyLeaflet>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyLeaflet>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyLeaflet>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyLeaflet>?) {
...
///
/// The `Leaflet` type acts as a reference-counted owner of an underlying `HdyLeaflet` instance.
...
open class Leaflet: Gtk.Container, LeafletProtocol {
    ...
    @inlinable public init(_ op: UnsafeMutablePointer<HdyLeaflet>) {
    ...
    /// i.e., ownership is transferred to the `Leaflet` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyLeaflet>) {
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyLeaflet`.
    /// i.e., ownership is transferred to the `Leaflet` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyLeaflet>) {
...
public enum LeafletPropertyName: String, PropertyNameProtocol {
    ...
    case child = "child"
    case childTransitionDuration = "child-transition-duration"
    ...
    case hhomogeneousFolded = "hhomogeneous-folded"
    case hhomogeneousUnfolded = "hhomogeneous-unfolded"
    ...
    case vhomogeneousFolded = "vhomogeneous-folded"
    case vhomogeneousUnfolded = "vhomogeneous-unfolded"
...
public enum LeafletSignalName: String, SignalNameProtocol {
    ...
    case notifyChildTransitionDuration = "notify::child-transition-duration"
    ...
    case notifyHhomogeneousFolded = "notify::hhomogeneous-folded"
    case notifyHhomogeneousUnfolded = "notify::hhomogeneous-unfolded"
    ...
    case notifyVhomogeneousFolded = "notify::vhomogeneous-folded"
    case notifyVhomogeneousUnfolded = "notify::vhomogeneous-unfolded"
...
public extension LeafletProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyLeaflet` instance.
    @inlinable var leaflet_ptr: UnsafeMutablePointer<HdyLeaflet>! { return ptr?.assumingMemoryBound(to: HdyLeaflet.self) }

    /// Gets the previous or next child that doesn't have 'navigatable' child
    /// property set to `false`, or `nil` if it doesn't exist. This will be the same
    ...
    @inlinable func getChildBy(name: UnsafePointer<gchar>!) -> Gtk.WidgetRef! {
        let rv = Gtk.WidgetRef(hdy_leaflet_get_child_by_name(leaflet_ptr, name))
        return rv
    }

    /// Returns the amount of time (in milliseconds) that
    /// transitions between children in `self` will take.
    @inlinable func getChildTransitionDuration() -> Int {
        let rv = Int(hdy_leaflet_get_child_transition_duration(leaflet_ptr))
        return rv
    }
    ...
    @inlinable func getChildTransitionRunning() -> Bool {
        let rv = ((hdy_leaflet_get_child_transition_running(leaflet_ptr)) != 0)
        return rv
    }

    /// Gets whether `self` is folded.
    @inlinable func getFolded() -> Bool {
        let rv = ((hdy_leaflet_get_folded(leaflet_ptr)) != 0)
        return rv
    }

    /// Gets whether `self` is homogeneous for the given fold and orientation.
    /// See `hdy_leaflet_set_homogeneous()`.
    @inlinable func getHomogeneous(folded: Bool, orientation: GtkOrientation) -> Bool {
        let rv = ((hdy_leaflet_get_homogeneous(leaflet_ptr, gboolean((folded) ? 1 : 0), orientation)) != 0)
        return rv
    }
    ...
    @inlinable func getInterpolateSize() -> Bool {
        let rv = ((hdy_leaflet_get_interpolate_size(leaflet_ptr)) != 0)
        return rv
    }
    ...
    @inlinable func getModeTransitionDuration() -> Int {
        let rv = Int(hdy_leaflet_get_mode_transition_duration(leaflet_ptr))
        return rv
    }

    /// Gets the type of animation that will be used
    /// for transitions between modes and children in `self`.
    @inlinable func getTransitionType() -> HdyLeafletTransitionType {
        let rv = hdy_leaflet_get_transition_type(leaflet_ptr)
        return rv
    }

    /// Gets the visible child widget.
    @inlinable func getVisibleChild() -> Gtk.WidgetRef! {
        let rv = Gtk.WidgetRef(hdy_leaflet_get_visible_child(leaflet_ptr))
        return rv
    }

    /// Gets the name of the currently visible child widget.
    @inlinable func getVisibleChildName() -> String! {
        let rv = hdy_leaflet_get_visible_child_name(leaflet_ptr).map({ String(cString: $0) })
        return rv
    }
    ...
    @inlinable func insertChildAfter<WidgetT: Gtk.WidgetProtocol>(child: WidgetT, sibling: WidgetT?) {
        hdy_leaflet_insert_child_after(leaflet_ptr, child.widget_ptr, sibling?.widget_ptr)
    ...
    @inlinable func navigate(direction: HdyNavigationDirection) -> Bool {
        let rv = ((hdy_leaflet_navigate(leaflet_ptr, direction)) != 0)
        return rv
    }

    /// Inserts `child` at the first position in `self`.
    @inlinable func prepend<WidgetT: Gtk.WidgetProtocol>(child: WidgetT) {
        hdy_leaflet_prepend(leaflet_ptr, child.widget_ptr)
    ...
    @inlinable func reorderChildAfter<WidgetT: Gtk.WidgetProtocol>(child: WidgetT, sibling: WidgetT?) {
        hdy_leaflet_reorder_child_after(leaflet_ptr, child.widget_ptr, sibling?.widget_ptr)
    ...
    @inlinable func set(canSwipeBack: Bool) {
        hdy_leaflet_set_can_swipe_back(leaflet_ptr, gboolean((canSwipeBack) ? 1 : 0))
    ...
    @inlinable func set(canSwipeForward: Bool) {
        hdy_leaflet_set_can_swipe_forward(leaflet_ptr, gboolean((canSwipeForward) ? 1 : 0))
    
    }

    /// Sets the duration that transitions between children in `self`
    /// will take.
    @inlinable func setChildTransition(duration: Int) {
        hdy_leaflet_set_child_transition_duration(leaflet_ptr, guint(duration))
    ...
    @inlinable func setHomogeneous(folded: Bool, orientation: GtkOrientation, homogeneous: Bool) {
        hdy_leaflet_set_homogeneous(leaflet_ptr, gboolean((folded) ? 1 : 0), orientation, gboolean((homogeneous) ? 1 : 0))
    ...
    @inlinable func set(interpolateSize: Bool) {
        hdy_leaflet_set_interpolate_size(leaflet_ptr, gboolean((interpolateSize) ? 1 : 0))
    ...
    @inlinable func setModeTransition(duration: Int) {
        hdy_leaflet_set_mode_transition_duration(leaflet_ptr, guint(duration))
    ...
    /// become current.
    @inlinable func setTransitionType(transition: HdyLeafletTransitionType) {
        hdy_leaflet_set_transition_type(leaflet_ptr, transition)
    ...
    @inlinable func set<WidgetT: Gtk.WidgetProtocol>(visibleChild: WidgetT) {
        hdy_leaflet_set_visible_child(leaflet_ptr, visibleChild.widget_ptr)
    ...
    @inlinable func setVisibleChild(name: UnsafePointer<gchar>!) {
        hdy_leaflet_set_visible_child_name(leaflet_ptr, name)
    ...
    @inlinable var canSwipeBack: Bool {
        ...
        /// 'navigatable' child property set to `true` via a swipe gesture
        nonmutating set {
            hdy_leaflet_set_can_swipe_back(leaflet_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
    ...
    @inlinable var canSwipeForward: Bool {
        ...
        /// 'navigatable' child property set to `true` via a swipe gesture.
        nonmutating set {
            hdy_leaflet_set_can_swipe_forward(leaflet_ptr, gboolean((newValue) ? 1 : 0))
        }
    }

    /// Returns the amount of time (in milliseconds) that
    /// transitions between children in `self` will take.
    @inlinable var childTransitionDuration: Int {
        /// Returns the amount of time (in milliseconds) that
        /// transitions between children in `self` will take.
        get {
            let rv = Int(hdy_leaflet_get_child_transition_duration(leaflet_ptr))
        ...
        /// will take.
        nonmutating set {
            hdy_leaflet_set_child_transition_duration(leaflet_ptr, guint(newValue))
        }
    }
    ...
    @inlinable var interpolateSize: Bool {
        ...
        /// visible child, according to the set transition duration.
        nonmutating set {
            hdy_leaflet_set_interpolate_size(leaflet_ptr, gboolean((newValue) ? 1 : 0))
        }
    }
    ...
    @inlinable var modeTransitionDuration: Int {
        ...
        /// will take.
        nonmutating set {
            hdy_leaflet_set_mode_transition_duration(leaflet_ptr, guint(newValue))
        }
    }

    /// Gets the type of animation that will be used
    /// for transitions between modes and children in `self`.
    @inlinable var transitionType: HdyLeafletTransitionType {
        /// Gets the type of animation that will be used
        /// for transitions between modes and children in `self`.
        get {
            let rv = hdy_leaflet_get_transition_type(leaflet_ptr)
        ...
        /// become current.
        nonmutating set {
            hdy_leaflet_set_transition_type(leaflet_ptr, newValue)
        }
    }
    ...
    @inlinable var visibleChild: Gtk.WidgetRef! {
        ...
        /// change back to the previously visible child.
        nonmutating set {
            hdy_leaflet_set_visible_child(leaflet_ptr, UnsafeMutablePointer<GtkWidget>(newValue?.widget_ptr))
        }
    }
    ...
    @inlinable var visibleChildName: String! {
        ...
        /// See `hdy_leaflet_set_visible_child()` for more details.
        nonmutating set {
            hdy_leaflet_set_visible_child_name(leaflet_ptr, newValue)
        }
    }

    @inlinable var parentInstance: GtkContainer {
        get {
            let rv = leaflet_ptr.pointee.parent_instance
            return rv
        }
    }
...
///
/// The `PreferencesGroupProtocol` protocol exposes the methods and properties of an underlying `HdyPreferencesGroup` instance.
/// The default implementation of these can be found in the protocol extension below.
...
///
public protocol PreferencesGroupProtocol: Gtk.BinProtocol, Gtk.BuildableProtocol {
        /// Untyped pointer to the underlying `HdyPreferencesGroup` instance.
    var ptr: UnsafeMutableRawPointer! { get }

    /// Typed pointer to the underlying `HdyPreferencesGroup` instance.
    var preferences_group_ptr: UnsafeMutablePointer<HdyPreferencesGroup>! { get }
...
///
/// The `PreferencesGroupRef` type acts as a lightweight Swift reference to an underlying `HdyPreferencesGroup` instance.
/// It exposes methods that can operate on this data type through `PreferencesGroupProtocol` conformance.
/// Use `PreferencesGroupRef` only as an `unowned` reference to an existing `HdyPreferencesGroup` instance.
///
...
public extension PreferencesGroupRef {
    /// Designated initialiser from the underlying `C` data type
    @inlinable init(_ p: UnsafeMutablePointer<HdyPreferencesGroup>) {
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Designated initialiser from a constant pointer to the underlying `C` data type
    @inlinable init(_ p: UnsafePointer<HdyPreferencesGroup>) {
        ptr = UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: p))
    }

    /// Conditional initialiser from an optional pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafeMutablePointer<HdyPreferencesGroup>?) {
        guard let p = maybePointer else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }

    /// Conditional initialiser from an optional, non-mutable pointer to the underlying `C` data type
    @inlinable init!(_ maybePointer: UnsafePointer<HdyPreferencesGroup>?) {
        guard let p = UnsafeMutablePointer(mutating: maybePointer) else { return nil }
        ptr = UnsafeMutableRawPointer(p)
    }
...
///
/// The `PreferencesGroup` type acts as a reference-counted owner of an underlying `HdyPreferencesGroup` instance.
...
open class PreferencesGroup: Gtk.Bin, PreferencesGroupProtocol {
    ...
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafeMutablePointer<HdyPreferencesGroup>) {
        super.init(cPointer: op)
    }
    ...
    /// i.e., ownership is transferred to the `PreferencesGroup` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(_ op: UnsafePointer<HdyPreferencesGroup>) {
        super.init(raw: UnsafeMutableRawPointer(UnsafeMutablePointer(mutating: op)))
    }
    ...
    /// Designated initialiser from the underlying `C` data type.
    /// Will retain `HdyPreferencesGroup`.
    /// i.e., ownership is transferred to the `PreferencesGroup` instance.
    /// - Parameter op: pointer to the underlying object
    @inlinable public init(retaining op: UnsafeMutablePointer<HdyPreferencesGroup>) {
        super.init(retainingCPointer: op)
    }
...
public extension PreferencesGroupProtocol {
    /// Return the stored, untyped pointer as a typed pointer to the `HdyPreferencesGroup` instance.
    @inlinable var preferences_group_ptr: UnsafeMutablePointer<HdyPreferencesGroup>! { return ptr?.assumingMemoryBound(to: HdyPreferencesGroup.self) }

    @inlinable func getDescription() -> String! {
        let rv = hdy_preferences_group_get_description(preferences_group_ptr).map({ String(cString: $0) })
        return rv
    }

    /// Gets the title of `self`.
    @inlinable func getTitle() -> String! {
        let rv = hdy_preferences_group_get_title(preferences_group_ptr).map({ String(cString: $0) })
        return rv
    }

    /// Sets the description for `self`.
    @inlinable func set(description: UnsafePointer<gchar>!) {
        hdy_preferences_group_set_description(preferences_group_ptr, description)
    
    }

    /// Sets the title for `self`.
    @inlinable func set(title: UnsafePointer<gchar>!) {
        hdy_preferences_group_set_title(preferences_group_ptr, title)
    ...
    @inlinable var description: String! {
        ...
        /// Sets the description for `self`.
        nonmutating set {
            hdy_preferences_group_set_description(preferences_group_ptr, newValue)
        }
    }
    ...
    @inlinable var title: String! {
        ...
        /// Sets the title for `self`.
        nonmutating set {
            hdy_preferences_group_set_title(preferences_group_ptr, newValue)
        }
    }
